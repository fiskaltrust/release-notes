...
In this version of the Middleware, we made it possible to run the German middleware on an ARM device, we've also fixed an issue in which explicit Fail-transaction calls in failed mode were not removing the failed start transactions in the database.
